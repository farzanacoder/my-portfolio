...
const ProjectCard = ({ image, title, category, link }) => {
    return (
        <Flex className='lg:w-[320px] flex-col gap-x-5 items-center bg-white py-5 rounded-2xl relative group/item'>
            ...
            <a href={link} target='_blank' rel='noopener noreferrer' className='absolute top-0 left-0 w-full h-full rounded-2xl'>
                <Flex className='w-full h-full rounded-2xl gap-x-4 justify-center items-center bg-gradient-to-l from-[#a4b0be] to-[#57606f] opacity-0 group-hover/item:opacity-70 transition-all duration-500 text-black text-2xl font-extrabold font-robo cursor-pointer'>
